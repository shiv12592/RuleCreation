    ...
    result[selectOperation] = conditions.map(condition => {
      if (condition.rows && condition.selectOperation) {
        ...
        if (condition.source === 'Request') {
          ...
          obj["Field"] = condition.field || ""; // Send empty string if field is not selected
        }
        return obj;
      }
    });
...
<select value={condition.identityAttribute} onChange={(e) => isGrouped ? handleChangeInner(index, i, 'identityAttribute', e.target.value) : handleChange(index, 'identityAttribute', e.target.value)}>
...
<select value={condition.locationAttribute} onChange={(e) => isGrouped ? handleChangeInner(index, i, 'locationAttribute', e.target.value) : handleChange(index, 'locationAttribute', e.target.value)}>
