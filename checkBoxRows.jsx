...
const RuleConditionRows = () => {
  ...
  const handleSubmit = () => {
    // Filter out the irrelevant fields from the JSON output based on the source value
    const filteredConditions = conditions.map((condition) => {
      if (condition.source === 'Request') {
        return {
          source: condition.source,
          requestAttribute: condition.requestAttribute,
          value: condition.value
        }
      } else if (condition.source === 'Identity') {
        return {
          source: condition.source,
          identityAttribute: condition.identityAttribute,
          value: condition.value
        }
      } else if (condition.source === 'Location') {
        return {
          source: condition.source,
          locationAttribute: condition.locationAttribute,
          value: condition.value
        }
      } else {
        return condition
    ...
    console.log(JSON.stringify({ [selectOperation]: filteredConditions }, null, 2))
